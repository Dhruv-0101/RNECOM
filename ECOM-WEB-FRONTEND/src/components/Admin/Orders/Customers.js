...
export default function Customers() {
  ...
  const customerUsers = customers?.map((customer) => customer?.user);
  const uniqueCustomers = Array.from(
    new Map(customerUsers?.map((user) => [user?.email, user])).values()
  );
  ...
  // });
  /*const removeDuplicateCustomers = (customers) => {
    const uniqueCustomers = [];
    const seenIds = {};
  
    customers?.forEach((customer) => {
      if (!seenIds[customer.id]) {
  ...
  // Get array with unique customers
